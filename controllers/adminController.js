...
export const RA_post = async (req, res) => {
  ...
  if (!req.user.type.includes("admin"))
    return res
      .status(403)
      .json({ E: "Your account can't perform this action" });
  ...
  try {
    ...
    if (status === "approved") {
      ...
      Emails(
        user,
        `Your webtoon titled ${toon.title} has been approved`,
        "Webtoon Approval"
      );
    } else {
      Emails(
        user,
        `Your webtoon titled ${toon.title} was rejected `,
        "Webtoon Approval"
      );
    }
    ...
    return res
      .status(500)
      .json({ E: "Error occurred performing action pls try it again" });
  }
};
...
export const BN_post = async (req, res) => {
  ...
  try {
    ...
    if (user.type === "admin1" || user.id === id)
      return res
        .status(403)
        .json({ E: "You can't perform this action on this account" });

    await user.updateOne({ status });
    let userz = await User.find();
    status === "active"
      ? Emails(user, "Your Account has been activated", "Account Activation")
      : Emails(
          user,
          "Your Account has been banned because u went againt our guidelines",
          "Account Ban"
        );
...
export const mkAdmin_post = async (req, res) => {
  ...
  if (!req.user.type.includes("admin1"))
    return res.status(403).json({ E: "You cant do this" });
  ...
  try {
    ...
    if (type === "admin2") {
      Emails(
        user,
        "Your Account type has been upgraded to Admin",
        "Account Upgrade"
      );
      return res
        .status(200)
        .json({ userz, M: "Account type updated to an Admin" });
    }
    Emails(user, "Your Admin Status has been removed", "Admin Removal");
    return res
      .status(200)
      .json({ userz, M: "Admin status has been removed from Account" });
  } catch (err) {
    return res.status(500).json({ E: "Error occured performing action" });
  }
};
...
export const scripture_post = async (req, res) => {
  ...
  try {
    ...
    if (scriptureList.length > 0)
      await scriptureList[0].updateOne({ book, word });
